...
require '../php/configBDD.inc.php';
	...
	$requete->execute(array($_SESSION['utilisateur']));
		$requete2=$PDO_BDD->prepare("SELECT * FROM `t_entreprise` where ID_USER=(SELECT ID_USER FROM `t_user` where UTI_MAIL=?)");
	...
	<body>
		<form action="../php/modif_profil.php" method="get" onSubmit="return validation(this)">
		  ...
		  <div class="form-group">
			...
			<input type="tel" class="form-control" name="tel_entreprise" pattern="[0][0-9]-[0-9]{2}-[0-9]{2}-[0-9]{2}-[0-9]{2}" value=<?php echo $p2['TEL']; ?> >
		  </div>
		  <div class="form-group">
			<label for="rs_sociale">Raison Sociale :</label>
			<input type="text" class="form-control" name="rs_sociale"  value=<?php echo $p2['RAISON_SOCIALE']; ?> >
		  </div>
		  <div class="form-group">
